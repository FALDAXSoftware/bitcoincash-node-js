      ...
      node(label) {
         ...
         container('build-container'){
              ...
              if (namespace) {
                  ...
                  s3Download(file:'.env', bucket:'env.faldax', path:"bitcoincash-node-js/${namespace}/.env", force:true)
                }
                sh "ls -a"
                sh "docker build -t ${imageRepo}/bitcoincash:${imageTag}  ."
                sh "docker push  ${imageRepo}/bitcoincash:${imageTag}"
                sh "helm upgrade --install --namespace ${namespace} --set image.tag=${imageTag} ${namespace}-bitcoincash-backend -f chart/values-${namespace}.yaml chart/"
              }
         }
         }
